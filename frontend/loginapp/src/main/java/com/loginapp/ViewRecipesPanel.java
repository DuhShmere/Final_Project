...
public class ViewRecipesPanel extends JPanel {
    ...
    private static class FetchResult {
        ...
        FetchResult(String fat, String protein, String carbs,
            ...
            this.fat = fat; this.protein = protein; this.carbs = carbs;
    ...
    private MealPlanData.Recipe findInMealPlanData(String mealName) {
        for (MealPlanData.MealPlan plan : MealPlanData.getAllPlans()) {
            for (MealPlanData.DayPlan day : plan.days) {
                if (day.breakfast.name.equals(mealName)) return day.breakfast;
                if (day.lunch.name.equals(mealName))     return day.lunch;
                if (day.dinner.name.equals(mealName))    return day.dinner;
            }
        }
        return null;
    }
    ...
    if (allowedIngredients == null || allowedIngredients.isEmpty()) return rawIngredients;

    // Load deselected ingredients directly
    List<String> deselected = db.loadDeselectedIngredients(username);
    if (deselected == null || deselected.isEmpty()) return rawIngredients;
    ...
    for (String part : parts) {
        ...
        if (!blocked) kept.add(part.trim());
    }

    if (kept.isEmpty()) return "(All ingredients filtered by your Settings preferences)";
    ...
    private JPanel makeMealRow(int index, String name, String category, String calories,
        ...
        if (!calories.equals("N/A")) {
            try {
                ...
                if (cal < 300)      calLabel.setForeground(new Color(0, 150, 0));
                else if (cal < 600) calLabel.setForeground(new Color(200, 130, 0));
                else                calLabel.setForeground(new Color(180, 0, 0));
